...
def main(args: Args) -> None:
    ...
    if args.anchor_sparse_dynamic_head_group_budget_table is not None:
        ...
        configure_dynamic_head_groups = getattr(
            diffusion_model,
            "configure_dynamic_packed_head_group_budget_table",
            None,
        )
        if configure_dynamic_head_groups is None:
            raise RuntimeError(
                "Loaded diffusion model does not support dynamic head-group budgets"
            )
        configure_dynamic_head_groups(
    ...
    if args.anchor_sparse_dynamic_action_history_table is not None:
        ...
        configure_dynamic_action_history = getattr(
            diffusion_model,
            "configure_dynamic_dense_action_history_table",
            None,
        )
        if configure_dynamic_action_history is None:
            raise RuntimeError(
                "Loaded diffusion model does not support dynamic action history"
            )
        configure_dynamic_action_history(
